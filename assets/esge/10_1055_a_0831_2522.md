...
In a post hoc analysis of a large clinical study with 15 345 person-years of follow-up, a cumulative incidence of 1.5% (95%CI


1.1%–2.0%) was found for metachronous CRC at 5 years, corresponding to an SIR of 1.6 (95%CI 1.2–2.2) as compared to the general population [33]. In a French study including 61 879 person-years of follow-up, the incidence of metachronous CRC following the diagnosis of an initial CRC was higher than expected (SIR 1.5, 95%CI 1.3–1.7). It remained higher throughout the study period, significantly so between the first and the fifth years following diagnosis (SIR 1.9, 95%CI 1.6–2.3) [58]. While direct comparisons are lacking, such excessive risk of metachronous cancer compared with the general population would appear to be of the same magnitude as that following resection of high risk precancerous lesions [53, 59].
...
```mermaid
flowchart TD
    A["Patients operated on colorectal cancer"] --> B["Preoperative colonoscopy"]
    B --> C{"Yes"}
    B --> D{"No"}
    D --> E["Colonoscopy at ≤ 6 months"]
    C --> F["1-year (from surgery)* colonoscopy"]
    E --> F
    F --> G["3-year (from the previous) colonoscopy"]
    G --> H["5-year (from the previous) colonoscopy"]
```
